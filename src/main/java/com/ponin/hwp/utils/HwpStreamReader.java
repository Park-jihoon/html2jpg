...
public class HwpStreamReader {
    ...
    /**
     * unsigned 4 byte array
     *
     * @param i
     * @return
     * @throws IOException
     */
    public long[] uint32(int i) throws IOException {
        if (i <= 0)
            throw new IllegalArgumentException();

        long[] uints = new long[i];
        for (int ii = 0; ii < i; ii++) {
            if (ensure(4) == 0)
                throw new EOFException();

            uints[ii] = LittleEndian.getUInt(buf);
        }
     ...
     * @throws IOException
     */
    public double[] doubles(int i) throws IOException {
        if (i <= 0)
            throw new IllegalArgumentException();
        ...
        for (int ii = 0; ii < i; ii++) {
            ...
            doubles[ii] = LittleEndian.getDouble(buf);
        }
